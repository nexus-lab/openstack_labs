...
\begin{lstlisting}
root@instance:~# cat /root/hello.txt
\end{lstlisting}
...
\end{lstlisting}
    ...
    \textbf{\texttildemid/set\_hostname} that matches the content shown below. Press \textbf{CTRL+X}, then \textbf{Y} to
    accept the file changes. Press \textbf{Enter} to confirm and exit back to the terminal.
\begin{lstlisting}
ubuntu@workstation: nano ~/set_hostname
\end{lstlisting}
