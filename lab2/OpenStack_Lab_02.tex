...
\begin{enumerate}
    ...
    \item Edit the \textbf{\texttildemid/keystonerc-cloud-dev} file and change the \texttt{OS\_USERNAME} from
    \textbf{admin} to \textbf{cloud-dev} and the \texttt{OS\_PROJECT\_NAME} from \textbf{demo} to \textbf{dev} using the
    \texttt{nano} command. Additionally, change the line beginning with \texttt{export PS1=...} so that
    \texttt{(keystone-admin)} becomes \texttt{(keystone-cloud-dev)}. Modify the file so the content matches below. When
    you are finished, press \textbf{CTRL+X}, then \textbf{Y} to accept the file changes. Press \textbf{Enter} to save
    and exit \textbf{\texttt{nano}}.
