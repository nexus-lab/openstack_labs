...
\begin{enumerate}
    ...
    \item Assign \textbf{cloud-dev} the \textbf{member} role in the \textbf{prod} project so that it can perform actions in that project.
    \begin{lstlisting}
        [ubuntu@workstation (keystone-admin)]:~$ openstack role add \
        > --project prod \
        > --user cloud-dev \
    ...
    \item Copy the keystone credentials file \textbf{\texttildemid/keystonerc-superuser} to \textbf{\texttildemid/keystonerc-cloud-dev}.
    \begin{lstlisting}
    ...
    \item Use \textbf{nano} to edit the \textbf{\texttildemid/keystonerc-cloud-dev} file.
...
\begin{enumerate}
    ...
    \item Use the \textbf{scp} command to copy the \textbf{\texttildemid/Downloads/key1.pem} file to the \textbf{devstack} machine.
...
\textbf{ubuntu}.
    ...
    \item SSH into the \textbf{ftp-server} instance using the \textbf{key1} private key.
    ...
    \item Restart the \textbf{vsftpd} service so that the changes will take effect.
    \begin{lstlisting}
    ...
    \item View the status of the \textbf{vsftpd} service.
