...
\end{lstlisting}
    ...
    \textbf{172.25.250.60} to \textbf{172.25.250.80}. Disable DHCP services for the subnet and use
...
\end{lstlisting}
    ...
    \item To review the implementation of the networks in the \textbf{devstack} machine, open a new terminal window and
    use the \textbf{\texttt{ssh}} command to connect to the \textbf{devstack} virtual machine as the \textbf{ubuntu}
    user.
\begin{lstlisting}
...
\begin{enumerate}
    ...
    \item Take note of the subnet ID for the \textbf{shared} subnet. To find the subnet ID, in the \textbf{Interfaces}
    tab of the \textbf{router1} router, click the \textit{Name} value in the same row as the \textbf{192.168.233.1} IP
    address. On the page this links to, the \textit{Subnet ID} can be found under the \textit{Fixed IPs} section. Copy
    or take a note of this ID for a future step.

    \begin{center}
        \includegraphics[width=\linewidth]{images/part2/step7a.png}
    \end{center}
    \begin{center}
        \includegraphics[width=\linewidth]{images/part2/step7b.png}
    \end{center}
...
\end{lstlisting}
    ...
    Substitute the subnet ID with the one you copied previously.
\begin{lstlisting}
ubuntu@workstation:~$ openstack router remove subnet router1 \
> 3bb68218-3879-4f34-8f09-1d182db8227d
\end{lstlisting}
...
\end{lstlisting}
    ...
    \item In the \textbf{Detach Interface} window, select \textbf{192.268.233.XYZ} and click \textbf{Detach Interface}.
