...
\input{../common.tex}
\begin{document}
\preamble{Lab 06: Deploying an FTP Server}
...
\begin{enumerate}
    ...
    \item Use \texttt{nano} to edit the \textbf{\texttildemid/keystonerc-cloud-dev} file. Change the
    \texttt{OS\_USERNAME} to \textbf{cloud-dev}. In the line beginning \texttt{export PS1=...}, replace
...
\begin{enumerate}
    ...
    \begin{lstlisting}
        [ubuntu@workstation (keystone-cloud-dev)]:~$ openstack server add \
        > floating ip ftp-server 172.25.250.27
    \end{lstlisting}
    ...
    \item View the status of the \texttt{vsftpd} service. If all changes were made correctly, it should report that the
