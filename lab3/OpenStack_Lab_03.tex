...
In this lab, you will manage images with the \textit{Horizon Dashboard} and \textit{OpenStack Unified CLI}, develop flavors, manage private networks, launch an internal instance, and verify the functionality of an internal instance.
...
\begin{enumerate}
    ...
    \item Enter \textbf{ubuntu1} into the \textit{Image Name} field.
    ...
    \item In the file browser, click \textbf{Downloads}, then select the \textbf{ubuntu.img} file.
    ...
    \item In the format dropdown, select \textbf{QCOW2 - QEMU Emulator}, and under \textit{Image Sharing}, set \textit{Visibility} to \textbf{Private}.
    ...
    \item To edit the image after it has been created, open the dropdown menu next to the \textbf{Launch} button in the row of \textbf{ubuntu1}, and click on \textbf{Edit Image}.
    ...
    \item Reopen the dropdown next to the \textbf{Launch} button in the same row as \textbf{ubuntu1}.
    ...
    \item Open a terminal if one is not already open, and source the \textbf{keystonerc-admin} file to load the \textbf{admin} user credentials.
    \begin{lstlisting}
    ...
    \item Try to delete the \textbf{ubuntu1} image while it is protected.
    ...
    \item Display the details of the \textbf{ubuntu2} image and verify that it was correctly updated.
...
\begin{enumerate}
    ...
    \item Create a subnet named \textbf{subnet2} for the \textbf{net2} network.
    ...
    \item The name of a network can also be changed.
    ...
    \item The \textbf{net3} network is now ready to be deleted, which will also delete its subnets.
...
\begin{enumerate}
    ...
    \item Click the $\uparrow$ button on the same row as the \textbf{flavor2} flavor to allocate that flavor, then click \textbf{Next}.
    \begin{center}
...
\begin{enumerate}
    ...
    \item Use the \textbf{free} command to ensure that the RAM and swap amount matches the one defined by the flavor, which is \textbf{1024 MB} for each.
    \begin{lstlisting}
    ...
    \item Use the \textbf{df} command to ensure that the instance has a disk size of \textbf{10 GB}, as defined by the flavor.
