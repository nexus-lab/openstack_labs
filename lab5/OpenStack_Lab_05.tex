...
\begin{enumerate}
    % TODO: Viewing the default Apache web page will require being able to access the instance
    ...
    \item In this lab, we will create our own router and external network. First, the default router and external
    network need to be deleted. List the routers to find the default router's name so it can be deleted.
    \begin{lstlisting}
    ...
    \begin{tipbox}
        When typing the command, make sure there is a space between \texttt{list} and the \texttt{\textbackslash}
        character, and press \textbf{Enter} to get the \texttt{>} and continue typing the rest of the command.
    \end{tipbox}

    \item Before the router can be deleted, it must be disconnected from all subnets. Show more details about
    \textbf{router1} to see the subnets to which it is connected.
    \begin{lstlisting}
    ...
    \item The previous command only outputs IDs, which can be used to remove the subnets from the router. However, to
    avoid having to type long ID strings, find the names of the subnets and remove them that way. List the subnets to
    see the which names are mapped to the IDs from the previous step.
    \begin{lstlisting}
    ...
    \textbf{ipv6-private-subnet} subnets. Remove \textbf{private-subnet} subnet from \textbf{router1}.
    \begin{lstlisting}
    ...
    \item Now, remove \textbf{ipv6-private-subnet} from \textbf{router1}.
    \begin{lstlisting}
        ubuntu@workstation:~$ openstack router remove subnet \
        > router1 \
        > ipv6-private-subnet
    ...
    \item Now, the router can be deleted. Delete \textbf{router1}.
    \begin{lstlisting}
    ...
    \begin{tipbox}
        While you cannot delete a router that still has connections to subnets through the OpenStack Unified CLI,
        deleting a router through the Horizon Dashboard will automatically remove its connections.
    \end{tipbox}
    ...
    \begin{tipbox}
        ...
        \texttt{openstack network show public}.
    \end{tipbox}

    \item Now, we will create our own resources. First, create an external network named \textbf{extern-net}. Set the
    network type to \textbf{flat} and the physical network to \textbf{public}. Set the network as shared and external.
    \begin{lstlisting}
    ...
    \item Create a subnet named \textbf{extern-subnet} in the \textbf{extern-net} network. Give the subnet a range of
    ...
    \item List the floating IPs to make sure none are already allocated. The list should be empty.
    \begin{lstlisting}
    ...
    \item From the floating IP pool in the \textbf{extern-net} network, create a floating IP.
    \begin{lstlisting}
    ...
    \item Create a router named \textbf{extern-router}.
    \begin{lstlisting}
    ...
    \item List the available subnets to see what the router can be connected to.
    \begin{lstlisting}
    ...
    \item View the permission of the \textbf{~/Downloads/dev-keypair.pem} file. You can see that the \textbf{ubuntu}
    user has read and write privileges to the file, the \textbf{ubuntu} user group has read and write privileges to the
    file, and other users have read permissions to the file. However, this is a sensitive file, and only the
    \textbf{ubuntu} user should have any privileges to it.
    \begin{lstlisting}
    ...
    \item Use the \texttt{chmod} command with a mode of \textbf{600} to make it so that the \textbf{ubuntu}
    user has read/write permissions on the file, and groups and other users have no permissions to the file.
    \begin{lstlisting}
    ...
    \item List the permissions again to ensure that the change took effect.
    \begin{lstlisting}
        [ubuntu@workstation (keystone-admin)]:~$ ls -l ~/Downloads/dev-keypair.pem
    \end{lstlisting}
    ...
    \item List the default security group rules.
    \begin{lstlisting}
    ...
    \begin{notebox}
        You can verify with the command \texttt{openstack security group rule show RULE\_ID} that the default rules
        allow any outgoing traffic over IPv4 or IPv6.
    \end{notebox}
...
\begin{enumerate}
    ...
    \item List the floating IPs to see the free address, which was disassociated from the previous instance when the
    ...
    \item Assign the floating IP to \textbf{instance2}.
    \begin{lstlisting}
