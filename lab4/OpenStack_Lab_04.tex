...
\begin{enumerate}
    ...
    \item Now, navigate to \textbf{Networks}.
    ...
    \item The next set of steps will show how to recreate the external network from the beginning of the lab using the CLI.
    ...
    \item List the networks again to see that \textbf{extern-net2} and \textbf{extern-subnet2} were created successfully.
    \begin{lstlisting}
        [ubuntu@workstation (keystone-admin)]:~$ openstack network list \
        > --max-width 100
    \end{lstlisting}
...
\begin{enumerate}
    ...
    \item Now, we will replicate the previous router using the CLI.
...
\begin{enumerate}
    ...
    \end{tipbox}
    \begin{tipbox}
        A floating IP address can be disassociated from and instance using the command \textbf{openstack floating ip remove INSTANCE FLOATING\_IP\_ADDRESS} and deleted using the command \textbf{openstack floating ip delete IP\_ADDRESS}.
    \end{tipbox}
...
\begin{enumerate}
    ...
    \item Select \textbf{All ICMP} from the \textit{Rule} dropdown and click \textbf{Add}.
...
\begin{enumerate}
    ...
    \begin{tipbox}
        For commands that output tables, you can pull out only the columns you want by using the \textbf{-c} option followed by the column name.
        This option can be chained as in the command above to list multiple columns.
    \end{tipbox}

    \item Use the \textbf{scp} command to send the \textbf{keypair2} key pair to the \textbf{devstack} machine using the SSH protocol.
