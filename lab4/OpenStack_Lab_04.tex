...
\begin{enumerate}
    ...
    \begin{tipbox}
        When typing the command, make sure there is a space between \texttt{extern-net2} and the
        \texttt{\textbackslash} character, and press \textbf{Enter} to get the \texttt{>} and continue typing the rest
        of the command.
    \end{tipbox}

    \begin{tipbox}
        To keep the output from overflowing while keeping the formatting intact, use the \texttt{--max-width n} option
        to limit the output to \texttt{n} characters per line. This is especially useful for commands that output long
...
\begin{enumerate}
    ...
    \begin{tipbox}
        You can use the \texttt{--floating-ip-address IP\_ADDRESS} argument to allocate a specific IP address.
    ...
    \end{tipbox}
    \begin{tipbox}
        A floating IP address can be disassociated from and instance using the command \texttt{openstack floating ip
        remove INSTANCE FLOATING\_IP\_ADDRESS} and deleted using the command \texttt{openstack floating ip delete
        IP\_ADDRESS}.
    \end{tipbox}
...
\begin{enumerate}
    ...
    namely the \texttt{ping} command, to reach instances in this security group.
...
\begin{enumerate}
    ...
    \begin{tipbox}
        For commands that output tables, you can pull out only the columns you want by using the \texttt{-c} option
        followed by the column name. This option can be chained as in the command above to list multiple columns.
    \end{tipbox}

    \item Use the \texttt{scp} command to send the \textbf{keypair2} key pair to the \textbf{devstack} machine using
